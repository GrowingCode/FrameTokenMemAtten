...
class BasicDecodeModel():
  ...
  def create_extra_default_metrics_meta(self):
    return [("skeleton_loss", tf.TensorShape(())), ("skeleton_accurate", tf.TensorShape([len(top_ks)])), ("skeleton_mrr", tf.TensorShape(())), ("skeleton_count", tf.TensorShape(())), ("sktacc_each", tf.TensorShape([len(top_ks)])), ("sktacc_whole", tf.TensorShape(())), ("sktacc_count", tf.TensorShape(()))]
